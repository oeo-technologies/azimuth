/*
 * Azimuth GPS Tracker Firmware V1.3.0 by OEO — Heltec WiFi LoRa 32 V4
 * =====================================================================
 * File version : v130
 * Changes      : LoRa peer location sharing.
 *                New BLE characteristic ac3 (LORA_PEER_CHAR) — NOTIFY only.
 *                Broadcasts own position over LoRa every N seconds.
 *                Receives peer position packets, stores up to LORA_MAX_PEERS.
 *                Peers expire after LORA_PEER_EXPIRE_MS without update.
 *                NVS keys: loraRegion, loraInterval.
 *                New BLE commands: SETLORA:region|interval, GETLORA.
 *                Device ID = last 3 bytes of MAC (stable unique identifier).
 *                Bumped FW_VERSION to v1.3.0.
 * =====================================================================
 * Board   : Heltec WiFi LoRa 32(V3) <- select this for V4 too
 * GPS     : L76K via onboard GNSS connector
 * Display : Onboard SSD1306 128x64 OLED
 * BLE     : NimBLE-Arduino 1.4.2 (must be 1.4.2, NOT 2.x)
 * LoRa    : RadioLib (SX1262 driver)
 *
 * Libraries (Arduino Library Manager):
 *  1. Heltec ESP32 Dev-Boards  (Heltec Automation)
 ...
 *  Upload Speed    : 921600
 *
 * Pages (no location saved):
 *  0=GPS  1=Info  2=Compass  3=Clock  4=Timer  5=Route  6=SaveLoc  7=Power
 * Pages (location saved):
 *  0=GPS  1=Info  2=Compass  3=Navigate  4=Clock  5=Timer  6=Route  7=SaveLoc  8=Power
 */
...
#define  BTN_DOUBLE_MS  400       // window for double press (ms)
...
bool fetchWeather() {
  if (strlen(owmApiKey) == 0 || !cachedFix) return false;
  char url[200];
...
void drawTitleScreen() {
  ...
  int16_t azX1, azY1, imX1, imY1;
  uint16_t azW, azH, imW, imH;

  // AZIMUTH title — moved down to y=22 to better centre without version line
  display.setFont(&FreeSansOblique12pt7b);
  display.getTextBounds("AZ", 0, 22, &azX1, &azY1, &azW, &azH);
  display.setFont(&FreeSansBold12pt7b);
  display.getTextBounds("IMUTH", 0, 22, &imX1, &imY1, &imW, &imH);

  int totalW = (int)azW + (int)imW;
  int startX = (128 - totalW) / 2 - (int)azX1;
  ...
  display.setFont(&FreeSansBold12pt7b);
  display.setCursor(startX + (int)azX1 + (int)azW - (int)imX1, 22);
  display.print("IMUTH");

  display.setFont(NULL);
  display.setTextSize(1);

  // Icons row — moved to y=31
  {
    const int iconY=31, iconW=8, iconGap=6;
    int totalIW = 5*iconW + 4*iconGap;
    int ix = (128-totalIW)/2;
    display.drawBitmap(ix,                   iconY, ICON_BT,  iconW, iconW, SSD1306_WHITE);
    display.drawBitmap(ix+(iconW+iconGap),   iconY, ICON_GPS, iconW, iconW, SSD1306_WHITE);
    display.drawBitmap(ix+2*(iconW+iconGap), iconY, ICON_SAT, iconW, iconW, SSD1306_WHITE);
    display.drawBitmap(ix+3*(iconW+iconGap), iconY, ICON_CMP, iconW, iconW, SSD1306_WHITE);
    display.drawBitmap(ix+4*(iconW+iconGap), iconY, ICON_SPD, iconW, iconW, SSD1306_WHITE);
  }

  // Slogan — centred, moved to y=48
  int16_t x1,y1; uint16_t wSub,hSub;
  const char* slogan = "Own your direction";
  display.getTextBounds(slogan, 0, 48, &x1, &y1, &wSub, &hSub);
  display.setCursor((128-(int)wSub)/2, 48);
...
void drawPageGPS() {
  ...
  // BLE icon top-right when connected
  if (bleConnected) display.drawBitmap(119, 1, ICON_BT, 8, 8, SSD1306_WHITE);

  // Battery top-right — show USB indicator when on USB power
  char batBuf[8];
  if (batOnUSB)
    ...
    snprintf(batBuf, sizeof(batBuf), "%d%%", batPercent);
  display.setCursor(83, 1); display.print(batBuf);
  drawBatIcon(107, 1, batPercent);

  if (!cachedFix) {
    // ── No fix — upward-facing satellite with sequential signal arcs ──
    // Usable area: y=10 (below header) to y=57 (above nav dots at y=58)
    // by=36 → body y=36–48, dish tip y=25, arc3 top y=25-15=10 ✓
    // text at y=49, bottom at y=55 — 3px clear of nav dots ✓

    const int cx = 64;
    const int by = 36;   // body top y

    // Solar panels
    display.fillRect(cx - 24, by + 3, 14, 7, SSD1306_WHITE);
    display.fillRect(cx + 10, by + 3, 14, 7, SSD1306_WHITE);

    // Body
    display.fillRect(cx - 9, by, 18, 12, SSD1306_WHITE);

    // Mast
    display.drawLine(cx,     by - 1, cx,     by - 6, SSD1306_WHITE);
    display.drawLine(cx - 1, by - 1, cx - 1, by - 6, SSD1306_WHITE);

    // Dish pointing up
    display.drawLine(cx - 7, by - 6, cx + 7, by - 6, SSD1306_WHITE);
    display.drawLine(cx - 7, by - 6, cx,     by - 11, SSD1306_WHITE);
    display.drawLine(cx + 7, by - 6, cx,     by - 11, SSD1306_WHITE);
    display.drawPixel(cx,     by - 8, SSD1306_WHITE);
    display.drawPixel(cx + 1, by - 8, SSD1306_WHITE);

    // Signal arcs — 4 states: 0=blank, 1=arc1, 2=arc1+2, 3=all three
    const int ax = cx;
    const int ay = by - 11;  // dish tip = y=25
    uint8_t arcPhase = (millis() / 350) % 4;

    if (arcPhase >= 1) {
      for (int deg = 200; deg <= 340; deg += 4) {
        float r = deg * M_PI / 180.0f;
        int px = ax + (int)(7  * cosf(r)), py = ay + (int)(7  * sinf(r));
        if (px >= 0 && px < 128 && py >= 10 && py < 58) display.drawPixel(px, py, SSD1306_WHITE);
      }
    }
    if (arcPhase >= 2) {
      for (int deg = 210; deg <= 330; deg += 3) {
        float r = deg * M_PI / 180.0f;
        int px = ax + (int)(11 * cosf(r)), py = ay + (int)(11 * sinf(r));
        if (px >= 0 && px < 128 && py >= 10 && py < 58) display.drawPixel(px, py, SSD1306_WHITE);
      }
    }
    if (arcPhase >= 3) {
      for (int deg = 215; deg <= 325; deg += 3) {
        float r = deg * M_PI / 180.0f;
        int px = ax + (int)(15 * cosf(r)), py = ay + (int)(15 * sinf(r));
        if (px >= 0 && px < 128 && py >= 10 && py < 58) display.drawPixel(px, py, SSD1306_WHITE);
      }
    }

    // "Acquiring..." — y=49, bottom at y=55, clear of nav dots at y=58
...
void drawPageInfo() {
  ...
  drawBatIcon(11, 48, batPercent);
  snprintf(buf, sizeof(buf), " %d%%  %.2fV", batPercent, batVoltage);
  display.setCursor(29, 48); display.println(buf);
}
...
void drawPageNavigate() {
  // Compass on RIGHT side — cx=96, divider at x=57, left panel=57px
  const int cx=93, cy=37, cr=20;
  float dist    = haversineDistance(cachedLat,cachedLon,savedLat,savedLon);
  float bearing = bearingTo(cachedLat,cachedLon,savedLat,savedLon);
  ...
  if (cachedFix) drawNeedle(cx,cy,cr-4,bearing);
  else { display.setCursor(cx-4,cy-3); display.println("--"); }

  // Vertical divider
  display.drawLine(57,0,57,63,SSD1306_WHITE);

  // Left panel — distance, cardinal, bearing, hint
  char buf[20];
  ...
  bool near = (dist < 20.0);
  if (near) {
    display.fillRect(0,10,56,18,SSD1306_WHITE);
    display.setTextColor(SSD1306_BLACK);
  }
  display.setCursor(2,18); display.println(buf);
  if (near) display.setTextColor(SSD1306_WHITE);

  display.setCursor(2,30); display.println(getCardinal(bearing));
  snprintf(buf,sizeof(buf),"%.0f deg",bearing);
  display.setCursor(2,40); display.println(buf);

  display.drawLine(0,50,56,50,SSD1306_WHITE);
  display.setCursor(2,53); display.println("Hold:clear");
}
...
void drawPageCountdown() {
  ...
  if (cdtSetMode) {
    ...
    display.setCursor(2, 39); display.println(buf);
    display.setCursor(2, 50); display.println("Press:preset Hold:done");
  } else if (cdtDone) {
    display.setCursor(2, 39); display.println("Timer complete");
    display.setCursor(2, 50); display.println("Hold: reset");
  } else if (cdtRunning) {
    display.setCursor(2, 39); display.println("Running...");
    display.setCursor(2, 50); display.println("Dbl: stop");
  } else {
    uint32_t setMin = cdtSetSecs / 60;
    snprintf(buf, sizeof(buf), "Set: %lu min", setMin);
    display.setCursor(2, 39); display.println(buf);
    display.setCursor(2, 50); display.println("Dbl:start Hold:set");
  }
}
...
void drawPagePower() {
  ...
  display.setCursor(44, 14); display.println("Hold 3s:");
  display.setCursor(44, 24); display.println("power off");
  display.setCursor(44, 38); display.println("Double tap:");
  display.setCursor(44, 48); display.println("Send SOS");
  ...
  if (state==LOW && btnLastState==LOW && !btnLongFired) {
    ...
    if (held >= threshold) {
      ...
      if (!screenOn) {
        ...
        // Navigate — clear waypoint → stay on same page (now Waypoint)
        locSaved=false; savedLat=0; savedLon=0;
        // currentPage stays the same — now shows Waypoint page
        oledDraw();
      } else if (currentPage==pageSaveLoc()) {
        // Waypoint page long press — mark waypoint then jump to Navigate
        if (cachedFix) {
          ...
          // currentPage stays — same slot now shows Navigate page
        }
        oledDraw();
...
void setup() {
  Serial.begin(115200);
  delay(300);
  Serial.println("[BOOT] Azimuth v3.0.7");
...
void loop() {
  ...
  if (currentPage==pageCompass() || (locSaved && currentPage==pageWaypoint()) || (wifiEnabled && currentPage==pageWeather())) oledInterval = 500;
